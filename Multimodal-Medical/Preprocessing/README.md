...
3. Download the authorized imaging, clinical, and annotation files directly
   from TCIA to a directory outside this Git checkout.
...
3. Generate de-identified image crops or embeddings locally.
...
5. Place the graph input needed for a local run under
   `MultiplexNetwork/data`. That directory is ignored by Git and is for local
   execution only.
